...
|     `n().set()`     | Index und Menge     | n("0 1").set(chord("C").voicing()) | Spielt Töne aus einer Menge von Tönen                                                           |
### b.  Mini-Notation
...
### Dokumentation
- Empfehlenswert ist es, im Seitenreiter unter `settings` `Enable tooltips on Ctrl and hover` zu aktivieren für interaktive Dokumentation
- Die [Strudel-Dokumentation](https://strudel.cc/workshop/getting-started/) ist bei Fragen sehr gut durchsuchbar. (Kürzel: `/` unter )
...
```
### Orbits & Rooms
Orbits sind sozusagen Sammelstellen für den Sound. Manche Effekte wie Halls/Rooms werden auf den gesamten Orbit angewendet. Standardmäßig landet alles Audio in Orbit 1. Es kann aber nützlich sein (bspw. verschiedene Halls) das Audio in einen anderen Orbit mit `.orbit(numberhere)` zu routen.
